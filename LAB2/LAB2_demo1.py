...
loader = DataLoader(TensorDataset(torch.from_numpy(train_data), torch.from_numpy(train_label)), batch_size=512, shuffle=True)

test_loader = DataLoader(TensorDataset(torch.from_numpy(test_data), torch.from_numpy(test_label)), batch_size=512, shuffle=False)
...
class DeepConvNet(torch.nn.Module):
    def __init__(self, C, N, filters = 25, activation_layer = torch.nn.ELU(alpha=1.0)):
        ...
        self.Dense = torch.nn.Linear(in_features=8600, out_features=N, bias=True)
...
models = [
    ...
    # DeepConvNet(C = 2, N = 2, filters=25, activation_layer=torch.nn.ReLU()).to(device=device),
    # DeepConvNet(C = 2, N = 2, filters=25, activation_layer=torch.nn.LeakyReLU()).to(device=device),
    # DeepConvNet(C = 2, N = 2, filters=25).to(device=device),
]
...
for model in models:
    ...
    epochs = 200 * 5 
    training(model, criterion, optimizer, loader, epochs=epochs, min_epoch = 800)
...
'''
# # LOAD -----------------------------------------------------------------------------------
best_leaky_eeg = EEG_net(C = 2, T = 750, activation_layer=torch.nn.LeakyReLU()).to(device=device)
best_leaky_eeg.load_state_dict(torch.load('best_model_parameters_demo1'))
criterion = torch.nn.CrossEntropyLoss()
