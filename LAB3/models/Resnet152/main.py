...
if __name__ == "__main__":
    ...
    models = [
        ...
        ResNet.ResNet152((3, None, None), filters=2).to(device=device)
    ]
    training_paras = [
   ...
   # -- train --
    ...
    for model, training_para in zip(models,training_paras):
        ...
        try:
            ...
            plot_acc_loss(all_activation_history, epochs=training_paras[0][2])
        except:
            model = best_model
            torch.save(model.state_dict(), "./models/resnet18.pth")
            exit(0)
    ...
    '''
    model = models[0]
    model.load_state_dict(torch.load("./models/best/resnet18.pth"))
