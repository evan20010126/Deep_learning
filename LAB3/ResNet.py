...
class bottleneck_block(torch.nn.Module):
    def __init__(self, x_channels, first_filters=64, strides=1) -> None:
        ...
        self.direct_layers = torch.nn.Sequential(
            torch.nn.Conv2d(x_channels, first_filters, kernel_size=(1,1), stride=strides),
            torch.nn.BatchNorm2d(first_filters, eps=1e-05, momentum=0.1, affine=True),  # Batch Normalization
            torch.nn.ReLU(),
            torch.nn.Conv2d(first_filters, first_filters, kernel_size=(3,3), stride=1, padding=1),
            torch.nn.BatchNorm2d(first_filters, eps=1e-05, momentum=0.1, affine=True),  # Batch Normalization
            torch.nn.ReLU(),
            torch.nn.Conv2d(first_filters, first_filters*4, kernel_size=(1,1), stride=1),
            torch.nn.BatchNorm2d(first_filters*4, eps=1e-05, momentum=0.1, affine=True),  # Batch Normalization
        )

        self.identity_layer = torch.nn.Sequential(torch.nn.Conv2d(x_channels, first_filters*4, kernel_size=1,stride=1))

        if strides != 1 :
            self.identity_layer = torch.nn.Conv2d(x_channels, first_filters*4, kernel_size=1, stride=strides)
...
class basic_block(torch.nn.Module):
    def __init__(self, x_channels, filters, strides=1, kernelSize=(3,3)):
        ...
        self.direct_layers = torch.nn.Sequential(
            ...
            torch.nn.BatchNorm2d(filters, eps=1e-05, momentum=0.1, affine=True),  # Batch Normalization
            torch.nn.ReLU(),
...
class ResNet18(torch.nn.Module):
    def __init__(self, input_shape, filters):
        ...
        self.conv2_x = torch.nn.Sequential(
            basic_block(filters, filters, 1, (3,3)), # stride=1 non-down sampling
            basic_block(filters, filters, 1, (3,3)),
        )
        self.conv3_x = torch.nn.Sequential(
            basic_block(filters, filters * 2, 2, (3,3)), # stride=2 down sampling
            basic_block(filters * 2, filters * 2, 1, (3,3)),
        )
        self.conv4_x = torch.nn.Sequential(
            basic_block(filters * 2, filters * 4, 2, (3,3)), # stride=2 down sampling
            basic_block(filters * 4, filters * 4, 1, (3,3)),
        )
        self.conv5_x = torch.nn.Sequential(
            basic_block(filters * 4, filters * 8, 2, (3,3)), # stride=2 down sampling
        ...
        self.Dense = torch.nn.Linear(in_features=filters * 8*1*1, out_features=1, bias=True)
...
class ResNet152(torch.nn.Module):
    def __init__(self, input_shape, filters):
        ...
        self.conv3_x = self._make_layer(bottleneck_block, filters*2, 8, stride=2)


        self.conv4_x = self._make_layer(bottleneck_block, filters*4, 36, stride=2)
    ...
    def _make_layer(self, block, first_filters, num_blocks, stride):
        ...
        layers = []
        layers.append(block(self.C, first_filters=first_filters, strides=stride))

        self.C = first_filters * 4

        for _ in range(num_blocks-1):
            layers.append(block(self.C, first_filters=first_filters, strides=1))
...
class ResNet50(torch.nn.Module):
    def __init__(self, input_shape, filters):
        ...
        self.conv3_x = torch.nn.Sequential(
            # x1
            bottleneck_block(filters * 2, first_filters=filters, strides=2), # stride=1 non-down sampling
            # x2
            bottleneck_block(filters * 4, first_filters=filters, strides=1), # stride=1 
            # x3
            bottleneck_block(filters * 4, first_filters=filters, strides=1), # stride=1 
    ...
    def _make_layer(self, block, first_filters, num_blocks, stride):
        ...
        # block(input_channel, first_filters, stride)
        layers = []
        layers.append(block(self.C, first_filters=first_filters, stride=stride))
