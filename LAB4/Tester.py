...
class Test_model(VAE_Model):
    ...
    def val_one_step(self, img, label, idx=0):
        # change tensor into (seq, B, C, H, W)
        ...
        self.make_gif(generated_frame[0], os.path.join(
            self.args.save_root, f'seq{idx}/pred_seq.gif'))
        self.make_gif(label_frame[0], os.path.join(
            self.args.save_root, f'seq{idx}/pose.gif'))
