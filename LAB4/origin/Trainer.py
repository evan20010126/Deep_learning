...
class kl_annealing():
    def __init__(self, args, current_epoch=0):
        ...
        # 使用 frange_cycle_linear 初始化 kl_anneal_ratio
        # self.kl_anneal_ratio_values = self.frange_cycle_linear(
        #     n_iter=self.kl_anneal_cycle,
        #     start=args.kl_anneal_ratio,
        #     stop=1.0,  # 最終的 kl_anneal_ratio 上限
        #     n_cycle=1,  # 單個循環
        #     ratio=1.0   # 不對 kl_anneal_ratio 進行縮放
        # )
    ...
    def update(self):
        ...
        #         self.kl_anneal_ratio = self.kl_anneal_ratio_values[self.current_epoch]
...
class VAE_Model(nn.Module):
    ...
    def training_one_step(self, img, label, adapt_TeacherForcing):
        ...
        for frame_num in range(label.size(1)):
            # Conduct Posterior prediction in Encoder
    ...
    def val_one_step(self, img, label):
        ...
        with torch.no_grad():
            ...
            for frame_num in range(label.size(1)):
                ...
                g_out = self.Decoder_Fusion(de_frame, de_pos, z)
                g_out = self.Generator(g_out)  # next frame
...
if __name__ == '__main__':
    ...
    parser.add_argument('--kl_anneal_type', choices=["Cyclical", "Monotonic"], type=str,
